...
export function generateTypeStubs(
  ...
  for (const [name, binding] of Object.entries(bindings)) {
    ...
    if (binding.outputSchema) {
      const outputType = jsonSchemaToTypeScript(binding.outputSchema, outputTypeName);
    ...
    const description =
      includeDescriptions && binding.description ? `/** ${binding.description} */\n` : "";
...
function schemaToType(schema: Record<string, unknown>): string {
  ...
  if (schemaType === "object" && schema.properties) {
    ...
    const required = new Set((schema.required as Array<string> | undefined) ?? []);
  ...
  if (schema.anyOf || schema.oneOf) {
    const variants = (schema.anyOf || schema.oneOf) as Array<Record<string, unknown>>;
