...
import { createEventAwareBindings, toolsToBindings } from "./bindings/tool-to-binding";
import { stripTypeScript } from "./strip-typescript";
import type { ServerTool, ToolExecutionContext } from "@tanstack/ai";
import type { CodeModeTool, CodeModeToolConfig, CodeModeToolResult, IsolateContext } from "./types";
...
export function createCodeModeTool(
  ...
  "execute_typescript"
> {
  const { driver, tools, timeout = 30000, memoryLimit = 128, getSkillBindings } = config;
  ...
  return definition.server(
    async (input, toolContext?: ToolExecutionContext): Promise<CodeModeToolResult> => {
      ...
      try {
        ...
        const eventAwareBindings = createEventAwareBindings(allBindings, emitCustomEvent);
