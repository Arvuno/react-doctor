...
export function otelMiddleware(options: OtelMiddlewareOptions): ChatMiddleware {
  ...
  const closeIterationSpan = (state: RequestState, ctx: ChatMiddlewareContext): void => {
    ...
    safeCall("otel.onSpanEnd", () =>
      onSpanEnd?.({ kind: "iteration", ctx, iteration } as OtelSpanInfo<"iteration">, span),
    );
    span.end();
  ...
  return {
    ...
    onStart(ctx) {
      safeCall("otel.onStart", () => {
        ...
        const spanOptions =
          safeCall("otel.onBeforeSpanStart", () => onBeforeSpanStart?.(info, baseOptions)) ??
          baseOptions;
    ...
    onChunk(ctx, chunk) {
      safeCall("otel.onChunk", () => {
        ...
        if (captureContent && state.assistantTextBuffer.length > 0) {
          ...
          const outputJson = JSON.stringify([{ role: "assistant", content: completion }]);
        ...
        if (captureContent) {
          const argsBody =
            typeof hookCtx.args === "string"
              ? hookCtx.args
              : (safeCall("otel.serializeToolArgs", () => JSON.stringify(hookCtx.args ?? null)) ??
                "[unserializable_tool_args]");
          const redactedArgs = redactContent(argsBody);
          const toolInputJson = JSON.stringify([{ role: "tool", content: redactedArgs }]);
        ...
        if (captureContent) {
          ...
          const body =
            typeof info.result === "string"
              ? info.result
              : (safeCall("otel.serializeToolResult", () => JSON.stringify(info.result ?? null)) ??
