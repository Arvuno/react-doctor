...
export interface StreamProcessorEvents {
  ...
  onToolCall?: (args: { toolCallId: string; toolName: string; input: any }) => void;
...
export class StreamProcessor {
  ...
  addToolResult(toolCallId: string, output: any, error?: string): void {
    ...
    const messageWithToolCall = this.messages.find((msg) =>
      msg.parts.some((p): p is ToolCallPart => p.type === "tool-call" && p.id === toolCallId),
    );
  ...
  areAllToolsComplete(): boolean {
    const lastAssistant = this.messages.findLast((m: UIMessage) => m.role === "assistant");

    if (!lastAssistant) return true;

    const toolParts = lastAssistant.parts.filter((p): p is ToolCallPart => p.type === "tool-call");
  ...
  processChunk(chunk: StreamChunk): void {
    ...
    switch (c.type) {
      ...
      case "STEP_STARTED":
        this.handleStepStartedEvent(chunk as Extract<StreamChunk, { type: "STEP_STARTED" }>);
        break;
  ...
  private ensureAssistantMessage(preferredId?: string): {
    ...
    if (preferredId) {
      ...
      if (existingMsg) {
        ...
        const lastPart =
          existingMsg.parts.length > 0 ? existingMsg.parts[existingMsg.parts.length - 1] : null;
  ...
  private handleTextMessageContentEvent(
    ...
    const shouldEmit = this.chunkStrategy.shouldEmit(chunkPortion, state.currentSegmentText);
   ...
   */
  private handleToolCallStartEvent(chunk: Extract<StreamChunk, { type: "TOOL_CALL_START" }>): void {
    ...
    const { messageId, state } = this.ensureAssistantMessage(targetMessageId ?? undefined);
    ...
    if (!existingToolCall) {
      ...
      const toolName = (chunk as { toolCallName?: string }).toolCallName ?? chunk.toolName;
   ...
   */
  private handleToolCallArgsEvent(chunk: Extract<StreamChunk, { type: "TOOL_CALL_ARGS" }>): void {
    const toolCallId = chunk.toolCallId;
   ...
   */
  private handleToolCallEndEvent(chunk: Extract<StreamChunk, { type: "TOOL_CALL_END" }>): void {
   ...
   */
  private handleRunStartedEvent(chunk: Extract<StreamChunk, { type: "RUN_STARTED" }>): void {
   ...
   */
  private handleRunFinishedEvent(chunk: Extract<StreamChunk, { type: "RUN_FINISHED" }>): void {
   ...
   */
  private handleRunErrorEvent(chunk: Extract<StreamChunk, { type: "RUN_ERROR" }>): void {
    this.hasError = true;
   ...
   */
  private handleStepStartedEvent(chunk: Extract<StreamChunk, { type: "STEP_STARTED" }>): void {
   ...
   */
  private handleStepFinishedEvent(chunk: Extract<StreamChunk, { type: "STEP_FINISHED" }>): void {
   ...
   */
  private handleCustomEvent(chunk: Extract<StreamChunk, { type: "CUSTOM" }>): void {
    ...
    if (this.events.onCustomEvent) {
      const toolCallId =
        chunk.value && typeof chunk.value === "object" ? chunk.value.toolCallId : undefined;
 ...
 */
export function createReplayStream(recording: ChunkRecording): AsyncIterable<StreamChunk> {
  return {
