...
export const isJsxAttributeValueTruthy = (attribute: EsTreeNodeOfType<"JSXAttribute">): boolean => {
  const value = attribute.value;
  ...
  if (isNodeOfType(value, "JSXExpressionContainer")) {
    ...
    if (isNodeOfType(expression, "Literal")) {
      if (expression.value === null || expression.value === false || expression.value === "")
        return false;
    }
    if (isNodeOfType(expression, "TemplateLiteral") && expression.expressions?.length === 0) {
      const allEmpty = expression.quasis?.every(
        (quasi: EsTreeNode) =>
...
export const hasAccessibleChild = (children: EsTreeNode[]): boolean => {
  ...
  for (const child of children) {
    ...
    if (isNodeOfType(child, "JSXElement")) {
      ...
      const dangerouslySetInnerHTML = findJsxAttributeIgnoreCase(
    ...
    if (
      isNodeOfType(child, "JSXFragment") &&
      child.children &&
      hasAccessibleChild(child.children)
    ) {
      return true;
    }
  }
...
export const isInteractiveElement = (tagName: string, attributes: EsTreeNode[]): boolean => {
  if (tagName === "a") {
