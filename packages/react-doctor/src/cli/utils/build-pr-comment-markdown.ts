...
const findWorstProject = (projects: JsonReportProjectEntry[]): JsonReportProjectEntry | null => {
  let worstProject: JsonReportProjectEntry | null = null;
...
Suppress with: \`${suppressionHint}\`
  ...
  const overflowCount = diagnostics.length - renderedCount;
  const overflowNote =
    overflowCount > 0
...
export const buildPrCommentMarkdown = (
  ...
  const headlineProject = findWorstProject(report.projects);
...
export const PR_COMMENT_MARKDOWN_MARKER = PR_COMMENT_MARKER;
